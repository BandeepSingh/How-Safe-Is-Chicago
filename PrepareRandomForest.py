...
predictions = model.predict(DataVal.map(lambda x: x.features))
Final2016Preds  = inputrddval.map(lambda (((Community,Year,Month),label),features): (Community,Year,Month,label)).zip(predictions)
Final2016Preds2=Final2016Preds.map(lambda ((Community,Year,Month,label),predictions):(Community,Year,Month,predictions))
Final2016Preds2.saveAsTextFile(output)
