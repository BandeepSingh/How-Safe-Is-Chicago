...
output=sys.argv[2]
#/Volumes/personal/uzmaa-irmacs/Chicago/data
...
result=sqlContext.sql("select Community,Year(LastYearDate) as LastYear,Month(LastYearDate) as LastYearMonth,"
                              " case when count(*)<=100 then 0"
                              "       when count(*)>100 and count(*)<=200 then 1"
                              "       when count(*)>200 and count(*)<=400 then 2"
                              "       when count(*)>400 and count(*)<=800 then 3"
                              "       else 4 end as NextYearCrimeLabel"
                      " from labelData  where Community!='' group by Community,Year(LastYearDate),Month(LastYearDate)")
